...
sort([]) -> [];
sort([H|T]) -> 
	% Эта часть создает новый список, в котором элементы списка T, для которых 
	% время публикации меньше времени публикации H
	sort([X || X <- T, rss_parse:get_item_time(X) < rss_parse:get_item_time(H)]) 
					++ [H] % После того, как хвост отсортирован, текущий элемент
					% Эта часть создает новый список, в котором элементы списка T, для 
					% которых время публикации больше или равно времени публикации H
					++ sort([X || X <-T , rss_parse:get_item_time(X) >= rss_parse:get_item_time(H)]).